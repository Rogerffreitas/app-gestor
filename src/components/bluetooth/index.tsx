import { PeripheralServices } from '@/src/types'
...
import { handleAndroidPermissions } from '@/src/services/BlePermissions'
...
const DEVICE_SERVICE_UUID = '18f0'
...
const BluetoothDemoScreen: React.FC = () => {
    ...
    const connectPeripheral = async (peripheral: Omit<Peripheral, 'advertising'>) => {
        try {
            if (peripheral) {
                ...
                console.log(peripheralData.characteristics, 'peripheralData.characteristics=======')
                        ...
                        if (characteristic.descriptors) {
                            for (const descriptor of characteristic.descriptors) {
                                try {
                                    ...
                                    console.log(`[readDescriptor] Descriptor ${data} for ${peripheral.id} `)
                                } catch (error) {
                                    console.error(
    ...
    const disconnectPeripheral = async (peripheralId: string) => {
        try {
            ...
            console.error(`[disconnectPeripheral][${peripheralId}] disconnectPeripheral error`, error)
        }
    }
    ...
    const startScan = async () => {
        ...
        if (state === 'off') {
            if (Platform.OS == 'ios') {
                Alert.alert('Enable Bluetooth', 'Please enable Bluetooth in Settings to continue.', [
    ...
    const write = async () => {
        ...
        if (bleService) {
            ...
            const qrUrl = `http://164.152.34.165:3333/api/v1/public/apontoamento/`

            const result = await encoder
                .initialize()
                .codepage('cp860') // Português
                .align('center')
                .bold(true)
                .line(`PLACA: {materialTransport.tate}`)
                .bold(false)
                .line(`ID: {materialTransport.id}`)
                .line('--------------------------------') // 32 traços
                .align('left')
                .line(`Desc: ary}`)
                .line(`Saida: {materialTransport.workRoutes.arrivalLocation}`)
                .line(`Dest: {materialTransport.workRoutes.departureLocation}`)
                .line(`Estaca: {materialTransport.deliveryPicket}`)
                .line(`Mat: materialTransport.material.name}`)
                .line(`Data: {formatarData(materialTransport.createdAt)}`)
                .line(`Dist:eledWithinTheWork).toFixed(2)} km`)
                .line('--------------------------------')
                .newline()
                .align('center')
                .qrcode(qrUrl, 2, 6, 'h') // URL, modelo, tamanho, correção de erro
                .newline()
                .line('Pressione para validar')
                .newline()
                .newline()
                .cut()
                .encode()
            const data = Array.from(new TextEncoder().encode(result))
            const teste = await BleManager.write(
