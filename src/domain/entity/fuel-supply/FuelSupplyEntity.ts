...
export class FuelSupplyEntity extends AbstratcEntity {
    ...
    public dtoToEntity(data: FuelSupplyDto): FuelSupplyEntity {
        if (!Object.values(FuelSupplyTypes).includes(data.supplyType)) {
            throw new Error(`O tipo de abastecimento é inválido: ${data.supplyType}.`)
        }
    ...
    public modelToEntity(data: FuelSupplyProps): FuelSupplyEntity {
        this._quantity = +data.quantity
        this._valuePerLiter = +data.valuePerLiter
    ...
    get maintenanceTrucksWorkEquipmentId(): string | undefined {
    ...
    validate(changeErrorFields: ChangeErrorFields) {
        ...
        if (!this._supplyType?.trim()) addError('type', 'Preencha o campo obrigatório')
        ...
        console.info('[FuelSupply] Entity valid')
    }
}
