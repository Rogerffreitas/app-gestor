...
import { ChangeErrorFields, ErrorMessages, InvoiceStatus } from '../../../types'
...
export class MaterialTransportEntity extends AbstratcEntity {
    ...
    public async modelToEntity(data: MaterialTransportModel): Promise<MaterialTransportEntity> {
        const entity = new MaterialTransportEntity()
        entity._workRoutes = await new WorkRoutesEntity().modelToEntity(await data.workRoutes())

        entity._transportVehicle = new TransportVehicleEntity().modelToEntity(await data.transportVehicle())

        entity._material = new MaterialEntity().modelToEntity(await data.material())
